    ...
    <form action="{{ route('admin.portfolio.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="title" class="form-control @error('title') border-danger @enderror"
                   value="{{ old('title') }}" placeholder="Contoh: E-Commerce Platform" required>
            @error('title')<div class="text-danger mt-1" style="font-size:.8rem;">{{ $message }}</div>@enderror
        </div>
        <div class="mb-3">
            <label class="form-label">Kategori *</label>
            <input type="text" name="category" class="form-control @error('category') border-danger @enderror"
                   value="{{ old('category') }}" placeholder="Web App / Website / Dashboard" required>
            @error('category')<div class="text-danger mt-1" style="font-size:.8rem;">{{ $message }}</div>@enderror
        </div>
        <div class="mb-3">
            <label class="form-label">Deskripsi *</label>
            <textarea name="description" rows="4" class="form-control @error('description') border-danger @enderror"
                      placeholder="Jelaskan proyek ini..." required>{{ old('description') }}</textarea>
            @error('description')<div class="text-danger mt-1" style="font-size:.8rem;">{{ $message }}</div>@enderror
        </div>
        ...
        <div class="mb-3">
            ...
            <div style="font-size:.75rem;color:var(--muted);margin-top:.3rem;">Format: JPG, PNG, WebP. Maks 2MB.</div>
        </div>
